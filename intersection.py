...
class Intersection:
    def __init__(self, capacity, spwan_rates=[0.3, 0.3, 0.3, 0.3]):
    ...
    def next_state(self, action):
        ...
        for i in range(4):
            if (r.random() < self.spawn_rates[i]):
...
def demo(qlp, demo_range=10, print_=True, spawn_rates=[0.3, 0.3, 0.3, 0.3]):
    cross = Intersection(qlp.capacity, spawn_rates)
    cost = 0
...
def demo_contrast(demo_range=10, print_=True, spawn_rates=[0.3, 0.3, 0.3, 0.3]):
    cross = Intersection(15, spawn_rates)
    cost = 0
    # break_flag = False
    for i in range(demo_range):
        if (print_):
        ...
        state = cross.cars
        for lane in state:
            # if (len(lane) > cross.capacity):
            #    break_flag = True
            cost += len(lane) * len(lane)
        # if (break_flag):
        #    print("contrast exceed!")
        # break
        if (print_):
            cross.print_state()
        action = (i % 6 == 0)
...
if (__name__ == '__main__'):
    ql = QLP(0.5, 0.4, 0.5, 1000000, 10)  # alpha epsilon gamma
    ...
    ql_data = []
    natural_data = []
    spawn_rates = [0.3, 0.1, 0.3, 0.1]
    for i in range(5000):
        print(i)
        ql_data.append(demo(ql, 500, False, spawn_rates))
        natural_data.append(demo_contrast(500, False, spawn_rates))
